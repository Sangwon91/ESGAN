...
class CellResNet:
    def __init__(self, *,
        ...
        self.res_name = ("res_unit_{}".format(i) for i in itertools.count())
    ...
    def _build(self):
        ...
        # Size = 16
        x = conv3d(x, kernel_size=7, filters=f, strides=2)
        # No pooling...
        for _ in range(3):
            x = self._residual_unit(x=x, filters=f)
        # Filters = 64
        f *= 2

        # Size = 8
        x = self._residual_unit(x=x, filters=f, reduce_size=True)
        for _ in range(3):
            x = self._residual_unit(x=x, filters=f)
        # Filters = 128
        f *= 2
        ...
        # No avg pool
        x = tf.layers.flatten(x)
        x = dense(x, units=3, use_bias=True)

        self.logits = x
        self.outputs = tf.nn.sigmoid(x)
    ...
    def _residual_unit(self, *, x, filters, reduce_size=False):
        with tf.variable_scope(next(self.res_name)):
            if reduce_size:
                x_in = conv3d(
                    x, kernel_size=1, filters=filters,
                    strides=2, name="reduce",
                )
                strides = 2
            else:
                x_in = x
            ...
            x = conv3d(
                    x, kernel_size=3, filters=filters,
                    strides=strides, name="conv3d_0",
                )
            x = batch_normalization(
                    x, training=self.training, name="batchnorm_0")
            x = tf.nn.relu(x)

            x = conv3d(
                    x, kernel_size=3, filters=filters,
                    strides=1, name="conv3d_1",
                )
            x = batch_normalization(
                    x, training=self.training, name="batchnorm_1")
            x += x_in
