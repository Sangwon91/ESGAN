...
class Encoder:
    ...
    def _build(self):
        ...
        with tf.variable_scope("encoding"):
            ...
            x = dense(x, units=self.config.h, activation=None)
...
class Decoder:
    ...
    def _build(self):
        ...
        with tf.variable_scope("h0"):
            h0 = tf.layers.dropout(z, rate=0.2, training=self.config.training)
            h0 = dense(h0, units=8*8*8*n, activation=None)
    ...
    def train(self, checkpoint=None):
        ...
        for i in range(train_steps):
            ...
            if i % self.config.save_every == 0:
                ...
                ops = [
                    self.merged_summary,
                    self.disc_fake.inputs,
                    self.disc_fake.outputs,
                    self.disc_real.inputs,
                    self.disc_real.outputs,
                ]

                summary, fake_in, fake_out, real_in, real_out = self.sess.run(
                    ops,
                    feed_dict=feed_dict,
                    options=run_options,
                ...
                for j, sample in enumerate(fake_in):
                    stem = "fake_in_{}".format(j)
                    write_visit_input(
                        stem, sample, self.config.size,
                        energy_range, self.sample_dir)

                for j, sample in enumerate(fake_out):
                    stem = "fake_out_{}".format(j)
                    write_visit_input(
                        stem, sample, self.config.size,
                        energy_range, self.sample_dir)
